...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // On successful registration, show success modal and then redirect.
      if (response.data) {
      ...
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-md flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full md:w-1/2 p-10 flex flex-col justify-center bg-gray-50">
          ...
          <div className="flex gap-6 mb-6 text-gray-600 font-medium">
            <Link to="/login" className="hover:text-gray-900 transition border-b-2 border-transparent hover:border-gray-900 pb-1">
              Login
            </Link>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <input type="text" placeholder="Full name" value={fullName} onChange={(e) => setFullName(e.target.value)} required className="w-full px-4 py-2 border rounded-lg bg-gray-100 text-gray-700 focus:ring-2 focus:ring-green-400 outline-none" />
            <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-2 border rounded-lg bg-gray-100 text-gray-700 focus:ring-2 focus:ring-green-400 outline-none" />
            <div className="relative">
                <input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="w-full px-4 py-2 border rounded-lg bg-gray-100 text-gray-700 focus:ring-2 focus:ring-green-400 outline-none" />
            ...
            <div className="relative">
                <input type={showConfirmPassword ? "text" : "password"} placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required minLength={6} className="w-full px-4 py-2 border rounded-lg bg-gray-100 text-gray-700 focus:ring-2 focus:ring-green-400 outline-none" />
            ...
            <div>
              ...
              <div className="flex items-center gap-6">
                ...
                <label className="flex items-center gap-2 text-gray-700">
                  <input type="radio" name="role" value="official" checked={role === "official"} onChange={() => setRole("official")} />
                  Public Official
                </label>
              </div>
            </div>
