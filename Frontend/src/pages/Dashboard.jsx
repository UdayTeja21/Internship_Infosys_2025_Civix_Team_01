...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAndProcessData = async () => {
      ...
      try {
        ...
        const successfulPetitionsCount = fetchedPetitions.filter(
          (p) => p.isMyPetition && p.status === 'approved'
        ).length;
  ...
  const getFilteredPetitions = () => {
    ...
    const filtered =
      selectedCategory === 'all' ? allPetitions : allPetitions.filter((p) => p.category === selectedCategory);
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {categories.map((category) => (
            <button key={category.id} onClick={() => setSelectedCategory(category.id)} className={getButtonClass(category)}>
              {category.label}
            </button>
          ))}
        </div>
      ...
      {showSignOutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-lg p-6 min-w-[320px] flex flex-col items-center">
            <span className="text-gray-800 mb-5 text-lg font-semibold">Are you sure you want to sign out?</span>
            <div className="flex gap-3">
              <button onClick={handleSignOut} className="px-5 py-2 bg-red-600 text-white rounded hover:bg-red-700">Confirm</button>
              <button onClick={() => setShowSignOutModal(false)} className="px-5 py-2 bg-gray-100 text-gray-700 rounded hover:bg-gray-200">Cancel</button>
            </div>
          </div>
        </div>
      )}
