...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    const { title, description, options, targetLocation, closeDate } = req.body;
...
router.get("/my-polls", authMiddleware, async (req, res) => {
  try {
    const officialPolls = await Poll.find({ createdBy: req.user.id }).sort({ createdAt: -1 });
    res.json(officialPolls);
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch polls" });
  }
